/* ============================================================
   BECOOL CRM — asset_view.js
   
   აგრეგატების სრული გვერდი:
   - მარცხნივ sidebar (კატეგორიები collapse/expand)
   - მარჯვნივ: asset header (ჩამოშლადი) + KPI strip + ტაბები
   - ტაბები: მთავარი | სერვ. ისტ. | მდგ. & გეგმა | ინვოისები | IoT/BMS | ბიბლიოთეკა
   
   გამოყენება:
   - index.html-ში <div id="asset-view"> შეცვალე ამ ფაილის HTML-ით
   - script tags-ში დაამატე: <script src="js/asset_view.js"></script>
   - ui.js-ში viewAssets(b) ფუნქციაში გამოიძახე: loadAssetView(b)
   ============================================================ */


/* ══════════════════════════════════════════════════════════════
   HTML INJECTION — asset-view სექციის HTML
   გამოიძახება ერთხელ, ინიციალიზაციისას
   ══════════════════════════════════════════════════════════════ */
...
function injectAssetViewCSS() {
    ...
    style.textContent = `
/* ── Layout ── */
#asset-view { display:block; }
.av-layout { display:grid; grid-template-columns:210px 1fr; min-height:calc(100vh - 80px); border:1px solid #e2e8f0; border-radius:1rem; overflow:hidden; background:#fff; }

/* ── Sidebar ── */
.av-sb { background:#0f2942; overflow-y:auto; }
...
/* ── Main ── */
.av-main { display:flex; flex-direction:column; overflow:hidden; }
...
/* Tab content */
.av-tab-content { padding:16px 18px; overflow-y:auto; flex:1; }
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
window._av = {
    branchAssets: [],   // ყველა asset ამ ფილიალზე
    currentType:  null, // არჩეული ტიპი sidebar-ში
    currentAsset: null, // გახსნილი asset
    detailsOpen:  false,
};


/* ══════════════════════════════════════════════════════════════
   INIT — გამოიძახება viewAssets(b)-დან
   ══════════════════════════════════════════════════════════════ */
async function loadAssetView(branch) {
    ...
    // HTML + CSS ინექცია (პირველ გამოძახებაზე)
    injectAssetViewCSS();
    injectAssetViewHTML();
...
function _avShowListPanel() {
    document.getElementById('av-list-panel').classList.remove('av-hidden');
    document.getElementById('av-detail-panel').classList.add('av-hidden');
...
function _avFillHeader(a) {
    ...
    // specs
    const specs = [
        ['სიმძლ.', a.cooling_capacity_kw ? `${a.cooling_capacity_kw} kW cool` : null],
        ['მაცივარ.', a.refrigerant_type ? `${a.refrigerant_type}${a.refrigerant_charge_kg ? ' · '+a.refrigerant_charge_kg+'kg' : ''}` : null],
    ...
    const detItems = [
        ['მოდელი (შიდ.)',    a.indoor_model  || a.model    || null],
        ['მოდელი (გარე)',    a.outdoor_model || null],
        ['S/N შიდ.',         a.indoor_serial || a.serial_number || null],
        ['S/N გარე',         a.outdoor_serial || null],
        ['ინსტ. თარიღი',    a.installation_date || null],
        ['გამოშ. წელი',     a.manufacture_year  || null],
        ['გარანტია',        a.warranty_until    || null],
        ['სერვ. ინტ.',      a.service_interval_days ? a.service_interval_days + ' დღე' : null],
        ['EER',             specs_full.eer  || null],
        ['COP',             specs_full.cop  || null],
        ['SEER',            specs_full.seer || null],
        ['SCOP',            specs_full.scop || null],
        ['კვება',           specs_full.voltage || (a.voltage ? `${a.voltage}V/${a.phase}` : null)],
        ['MCA / MOCP',      specs_full.mca ? `${specs_full.mca}A / ${specs_full.mocp}A` : null],
        ['სითხ. მილი',     specs_full.liquid_line_inch || null],
        ['გაზ. მილი',      specs_full.gas_line_inch    || null],
    ].filter(([,v]) => v);
...
function _avFillKpiStrip(a) {
    ...
    const items = [
        { v: a.status === 'Operational' ? 'OK' : a.status, l: 'სტატუსი', cls: a.status === 'Operational' ? 'av-ks-ok' : 'av-ks-warn' },
    ...
    <div class="av-grid-2">
      ...
      <div class="av-tcard">
        ...
        <div class="av-sched-row">
          ...
          <span class="av-sched-task">ფილტ. + leak test</span>
        ...
        <div style="margin-top:12px;padding-top:10px;border-top:1px solid #f1f5f9">
          <div style="display:flex;justify-content:space-between;font-size:11px;color:#94a3b8;margin-bottom:5px">
            <span>Unit Health</span><span style="color:${(a.condition_score||5)>=7?'#166534':'#92400e'};font-weight:700">${a.condition_score||'—'} / 10</span>
          </div>
      ...
      <div class="av-tcard">
        ...
        <div class="av-grid-2" style="gap:6px">
          <div class="av-meas ${lastMeas.suction_pressure<10?'g':'y'}"><div class="av-meas-l">LP</div><div class="av-meas-v">${lastMeas.suction_pressure||'—'}<span class="av-meas-u">Bar</span></div></div>
          <div class="av-meas ${lastMeas.discharge_pressure<30?'g':'y'}"><div class="av-meas-l">HP</div><div class="av-meas-v">${lastMeas.discharge_pressure||'—'}<span class="av-meas-u">Bar</span></div></div>
          <div class="av-meas ${lastMeas.superheat>=4&&lastMeas.superheat<=8?'g':'y'}"><div class="av-meas-l">SH</div><div class="av-meas-v">${lastMeas.superheat||'—'}<span class="av-meas-u">K</span></div></div>
          <div class="av-meas g"><div class="av-meas-l">კომპ. A</div><div class="av-meas-v">${lastMeas.comp_current_a||'—'}<span class="av-meas-u">A</span></div></div>
        </div>
      ...
      <!-- ინვოის preview -->
      <div class="av-tcard">
        <div class="av-tcard-h">ბოლო ინვოისები <span class="av-tcard-ha" onclick="avSwitchTab('inv',null)">ყველა →</span></div>
        <div class="av-inv-head"><div>ნომ.</div><div>სამ.</div><div style="text-align:right">₾</div><div>სტ.</div></div>
        <div class="av-empty">ინვოისები: — (მალე)</div>
      </div>
...
async function _avRenderSvc() {
    ...
    tc.innerHTML = `
    <div class="av-tcard">
      <div class="av-tcard-h">სერვისის სრული ისტორია <span class="av-tcard-ha" onclick="avAddService()">+ სერვისი</span></div>
...
function _avRenderInv() {
    ...
    <div class="av-tcard">
      ...
      <div class="av-inv-head"><div>ნომ.</div><div>სამ.</div><div style="text-align:right">₾</div><div>სტ.</div></div>
      <div class="av-empty">ინვოისების ცხრილი — მალე</div>
    </div>`;
}
...
function _avRenderIot() {
    ...
    <div class="av-grid-3" style="margin-bottom:12px">
      ...
      <div class="av-iot-cell"><div class="av-live warn"></div><div class="av-iot-l">TC discharge</div><div class="av-iot-v">—<span class="av-iot-u">°C</span></div></div>
      ...
      <div class="av-iot-cell"><div class="av-live ok"></div><div class="av-iot-l">ამბ. ტემპ.</div><div class="av-iot-v">—<span class="av-iot-u">°C</span></div></div>
    </div>
    <div class="av-tcard">
      <div class="av-tcard-h">BMS კავშირი</div>
      <div style="font-size:12px;color:#64748b">Device ID: <b style="color:#0f172a">${a.iot_device_id||'—'}</b> · პროტ: BACnet/IP · სტ: <span style="color:#94a3b8">—</span></div>
    </div>`;
}
...
function _avRenderLib() {
    ...
    const docs = [
        ...
        { name:'ინსტ. სახელმ.', type:'PDF', icon:'#10b981', url: null },
        { name:'Wiring / სქემა', type:'ZIP', icon:'#f59e0b', url: a.wiring_diagram_url },
        { name:'სხვა ფაილები', type:'—', icon:'#94a3b8', url: null },
    ];
    tc.innerHTML = `
    <div class="av-grid-4">
      ${docs.map(d => `
...
function avOpenNewAsset() {
    if (window._av.currentType) {
        openAssetModal({ asset_type: window._av.currentType });
    } else {
        openAssetModal();
    }
}
