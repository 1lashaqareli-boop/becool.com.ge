/* ============================================================
   BECOOL CRM — api.js (სრული ვერსია - დაუკლებელი)
   ============================================================ */
        ...
        <div onclick="handleAssetClick('${a.id}')" 
            ...
            <div class="p-8">
                <div class="flex justify-between items-start mb-4">
                    <div><h4 class="text-2xl font-black text-slate-900 tracking-tighter italic uppercase">${a.name}</h4><p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">${a.brand} ${a.model || ''}</p></div>
                ...
                <div class="flex flex-col gap-2" onclick="event.stopPropagation()">
                    <button onclick='viewServiceLogs(${JSON.stringify(a)})' class="w-full bg-orange-500 text-white py-4 rounded-2xl text-[9px] font-black uppercase hover:bg-orange-600 transition flex items-center justify-center gap-2 italic">სერვისის ისტორია <i data-lucide="history" class="w-4 h-4"></i></button>
                </div>
            </div>
        </div>`).join('');
    lucide.createIcons();
...
async function saveCustomer() {
    ...
    const payload = {
        name, customer_type: isCompany ? 'Company' : 'Person', status: document.getElementById('c-status').value,
        legal_address: document.getElementById('c-legal-address').value, website: document.getElementById('c-website').value,
        bank_name: document.getElementById('c-bank').value, account_number: document.getElementById('c-account').value,
        image_url: imageUrl, phone: document.getElementById('c-phone').value, notes: document.getElementById('c-notes').value
    };
    const { error } = id ? await _supabase.from('customers').update(payload).eq('id', id) : await _supabase.from('customers').insert([payload]);
    if (!error) { closeModals(); loadCustomers(); } else { alert(error.message); }
}
    ...
    document.getElementById('branches-list').innerHTML = data.map((b, i) => `
        <div class="bg-white rounded-[3.5rem] shadow-xl border border-slate-100 overflow-hidden flex flex-col md:flex-row min-h-[500px] group transition-all hover:shadow-2xl">
            <div class="w-full md:w-96 relative bg-slate-100 overflow-hidden border-r border-slate-50">
                ${b.image_url ? `<img src="${b.image_url}" class="w-full h-full object-cover">` : `<div class="h-full flex items-center justify-center text-slate-300 font-black italic text-[10px]">OB ფოტო არაა</div>`}
                <div id="map-${i}" class="absolute bottom-8 left-8 right-8 h-40 rounded-[2rem] shadow-2xl border-4 border-white overflow-hidden"></div>
            </div>
            <div class="p-12 flex-1 flex flex-col">
                ...
                <button onclick='openBranchModal(${JSON.stringify(b)})' class="text-blue-500 font-black text-[11px] uppercase italic">რედაქტირება</button></div>
                <h3 class="text-5xl font-black text-slate-900 leading-none mb-3 italic tracking-tighter uppercase">${b.name}</h3>
                <p class="text-xs text-slate-400 font-bold mb-12 uppercase tracking-widest">📍 ${b.address || 'მისამართი არაა'}</p>
                <div class="flex gap-4 mt-auto">
                    <button onclick='viewAssets(${JSON.stringify(b)})' class="flex-[2] bg-slate-900 text-white font-black py-4 rounded-3xl text-[10px] uppercase hover:bg-blue-600 transition">აგრეგატები (ASSETS)</button>
...
async function saveBranch() {
    ...
    const payload = {
        customer_id: activeCustomer.id, name: document.getElementById('b-name').value.trim(), is_active: document.getElementById('b-active').value === 'true',
        address: document.getElementById('b-address').value, lat: parseFloat(document.getElementById('b-lat').value), lng: parseFloat(document.getElementById('b-lng').value),
        image_url: imageUrl, contact_person: document.getElementById('b-pers').value, contact_phone: document.getElementById('b-phon').value
    };
...
async function saveAsset() {
    ...
    let techSpecs = {}; try { techSpecs = JSON.parse(document.getElementById('a-tech').value); } catch(e) {}
    const payload = {
        branch_id: activeBranch.id, customer_id: activeCustomer.id, name: document.getElementById('a-name').value,
        asset_type: document.getElementById('a-type').value, brand: document.getElementById('a-brand').value,
        model: document.getElementById('a-model').value, image_url: imageUrl, technical_specs: techSpecs, status: document.getElementById('a-status').value
    };
...
async function loadServiceLogs() {
    ...
    document.getElementById('service-logs-list').innerHTML = data.map(log => `
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl flex gap-8">
            <div class="md:w-48 text-center border-r border-slate-50 pr-8">
                <span class="text-3xl font-black text-slate-900 italic">${new Date(log.service_date).toLocaleDateString('ka-GE', {day:'2-digit', month:'short'})}</span>
            </div>
            <div class="flex-1">
                <h4 class="text-xl font-black italic uppercase">${log.technician_name}</h4>
                <p class="text-sm text-slate-600 mt-2">${log.job_description}</p>
            </div>
        </div>`).join('');
    lucide.createIcons();
}
...
async function saveServiceLog() {
    const payload = {
        asset_id: activeAsset.id, branch_id: activeBranch.id, technician_name: document.getElementById('sl-tech').value,
        service_date: document.getElementById('sl-date').value, service_type: document.getElementById('sl-type').value,
        job_description: document.getElementById('sl-desc').value, media_urls: uploadedMediaUrls
    };
    const { error } = await _supabase.from('service_logs').insert([payload]);
    if (!error) { closeModals(); loadServiceLogs(); } else { alert(error.message); }
}

async function deleteData(table, id) {
    if (confirm('ნამდვილად გსურთ წაშლა?')) {
        const { error } = await _supabase.from(table).delete().eq('id', id);
        if (!error) { if (table === 'customers') loadCustomers(); else if (table === 'branches') loadBranches(); else if (table === 'assets') loadAssets(); else loadServiceLogs(); }
    }
}
