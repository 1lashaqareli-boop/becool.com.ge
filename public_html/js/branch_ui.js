/* ============================================================
   BECOOL CRM — branch_ui.js (სრული, გამართული ვერსია)
   FIX 1: overview-ში ბოლო სერვისები ჩანს
   FIX 2: overview-ის ფილიალების სიაში იგივე toolbar
   FIX 3: ფილ. ბარათზე დაჭერა → viewBranchDashboard (არა viewAssets)
   FIX 4: assets ტაბში ჩანაწერზე დაჭ. → loadAssetView
   FIX 5: services ტაბი — სრული query
   FIX 6: loadCustomerKPIs realtime refresh
   FIX 7: breadcrumb — "← ფილიალები" ღილაკი asset dashboard-ზე
   ============================================================ */

let _branchesData    = [];
let _branchRT        = null;
let overviewMap      = null;
let _overviewBranches = []; /* overview-ში cached branches */

/* ══════════════════════════════════════════════════════════════
   BREADCRUMB
   ══════════════════════════════════════════════════════════════ */
window._nav = { customer: null, branch: null };
...
function _renderBreadcrumb() {
    ...
    let html = `<div class="container mx-auto flex items-center gap-2 h-9 overflow-x-auto" style="scrollbar-width:none">
        <button onclick="showCustomers()" class="flex items-center gap-1.5 text-slate-400 hover:text-white transition text-xs font-medium">
    ...
    if (c) {
        ...
        if (b) {
            html += `<button onclick="backToCustomer()" class="flex items-center gap-1.5 text-slate-400 hover:text-white transition text-xs font-medium truncate max-w-[150px]">
                ${c.image_url ? `<img src="${c.image_url}" class="w-4 h-4 rounded object-cover flex-shrink-0">` : `<div class="w-4 h-4 rounded bg-blue-600 flex items-center justify-center text-[8px] font-black text-white flex-shrink-0">${(c.name||'BC').substring(0,2).toUpperCase()}</div>`}
            ...
            html += `<span class="flex items-center gap-1.5 text-white font-bold text-xs truncate max-w-[200px]">
                ${c.image_url ? `<img src="${c.image_url}" class="w-4 h-4 rounded object-cover flex-shrink-0">` : `<div class="w-4 h-4 rounded bg-blue-600 flex items-center justify-center text-[8px] font-black text-white flex-shrink-0">${(c.name||'BC').substring(0,2).toUpperCase()}</div>`}
                <span class="truncate">${c.name||'---'}</span>
            </span>`;
        }
...
/* ── FIX 7: viewBranchDashboard — breadcrumb განახლება ── */
...
/* ══════════════════════════════════════════════════════════════
   FIX 6: loadCustomerKPIs — realtime-ზე გამოძახება
   ══════════════════════════════════════════════════════════════ */
async function loadCustomerKPIs() {
    ...
    set('kpi-assets',    ac);
    set('kpi-services',  sc);
    set('kpi-last-service', ls?.[0] ? new Date(ls[0].service_date).toLocaleDateString('ka-GE',{day:'2-digit',month:'short',year:'numeric'}) : '—');
}

/* ══════════════════════════════════════════════════════════════
   customer details toggle / edit / delete
   ══════════════════════════════════════════════════════════════ */
let _cvOpen = true;
function toggleCustomerDetails() {
    _cvOpen = !_cvOpen;
    const d = document.getElementById('cv-details');
    const i = document.getElementById('cv-toggle-icon');
...
/* FIX 3: viewAssets → ახლა მხოლოდ "აგრეგ." ღილაკიდან */
...
/* ══════════════════════════════════════════════════════════════
   FIX 1 + 2: OVERVIEW TAB
   FIX 1 — ბოლო სერვისები ჩანს
   FIX 2 — ფილიალების სიაში toolbar (ძებნა + სორტ.)
   FIX 3 — ფილ. ბარათზე → viewBranchDashboard
   ══════════════════════════════════════════════════════════════ */
    ...
    if (sEl) {
        ...
        if (svc && svc.length > 0) {
            sEl.innerHTML = svc.map(s=>`
                <div class="flex items-start gap-2 py-2 border-b border-slate-50 last:border-0 hover:bg-slate-50 rounded px-1 cursor-pointer transition" onclick='_openSvcDetail(${JSON.stringify(s)})'>
...
function _injectOverviewBranchToolbar() {
    ...
    if (!tb) {
        ...
        tb.id = 'overview-br-toolbar';
        tb.className = 'flex items-center gap-2 mb-3 flex-wrap';
        blEl.parentNode.insertBefore(tb, blEl);
    }
    tb.innerHTML = `
...
async function loadLiveMapTab() {
    ...
    setTimeout(()=>{
        const cont = document.getElementById('live-map-container');
        if (!cont) return;
...
/* ══════════════════════════════════════════════════════════════
   BRANCHES TAB — სრული toolbar: ძებნა, სორტ. ანბ./ზომ., ფილტ., Excel, RT
   ══════════════════════════════════════════════════════════════ */
...
/* FIX 3 — ბარათზე onclick → viewBranchDashboard */
function _renderBranches(data) {
    ...
    cont.innerHTML=data.map((b,i)=>{
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col md:flex-row mb-4 group hover:shadow-lg transition-all cursor-pointer"
             onclick='viewBranchDashboard(${bJson})'>
            ...
            <div class="flex-1 p-5 flex flex-col justify-between">
                <div>
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <button onclick='event.stopPropagation();openBranchModal(${bJson})'
                            class="text-blue-500 text-[11px] font-bold hover:underline">რედ. →</button>
                    </div>
                ...
                <div class="flex gap-2 mt-3">
                    <button onclick='event.stopPropagation();viewAssets(${bJson})'
...
/* FIX 6: Realtime → KPI-ც განახლდება */
...
function exportBranchesExcel() {
    ...
    a.download=`branches_${activeCustomer?.name||'export'}_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();URL.revokeObjectURL(url);
}

/* ══════════════════════════════════════════════════════════════
   FIX 4: ASSETS TAB — row click → loadAssetView
   ══════════════════════════════════════════════════════════════ */
async function loadAllAssets() {
    ...
    let q=_supabase.from('assets').select('*,branches(name,id)').eq('customer_id',activeCustomer.id);
    ...
    <tbody>
    ${data.map(a=>{
        /* FIX 4 — onclick → viewBranchDashboard + assets ტაბი */
        const bObj=JSON.stringify({id:a.branch_id,name:a.branches?.name||'',lat:null,lng:null});
        return `<tr class="border-b border-slate-50 hover:bg-slate-50 cursor-pointer"
                    onclick='_openAssetFromTable(${JSON.stringify(a)})'>
            <td class="py-2.5 px-2 font-medium text-slate-800">${a.name||'—'}</td>
            <td class="py-2.5 px-2 text-slate-500">${a.asset_type||'—'}</td>
            <td class="py-2.5 px-2">
                <span class="px-2 py-0.5 rounded-full font-bold text-[9px] ${clC[a.asset_class]||'bg-slate-100 text-slate-500'}">${a.asset_class||'—'}</span>
            </td>
            <td class="py-2.5 px-2 text-slate-500">${a.branches?.name||'—'}</td>
            <td class="py-2.5 px-2">
                <span class="px-2 py-0.5 rounded-full font-bold text-[10px] ${sC[a.status]||'bg-slate-100 text-slate-500'}">${sL[a.status]||a.status}</span>
            </td>
...
/* FIX 4 — asset სტრიქონიდან → branch-dashboard assets ტაბი */
async function _openAssetFromTable(a) {
    /* ვიღებ ფილიალის სრულ ობიექტს */
    const {data:branch} = await _supabase.from('branches').select('*').eq('id',a.branch_id).single();
    ...
    if (typeof _doViewBranchDashboard === 'function') {
        ...
        /* assets ტაბზე გადართვა */
        setTimeout(()=>{
            const assetsTab = [...document.querySelectorAll('#bdc-nav .bdc-tab')].find(t=>t.textContent.trim().includes('აგრეგ'));
            if (assetsTab) switchBdcTab('assets', assetsTab);
        }, 300);
    }
}
...
/* ══════════════════════════════════════════════════════════════
   FIX 5: SERVICES TAB — სრული query + render
   ══════════════════════════════════════════════════════════════ */
async function loadAllServices() {
    ...
    let q=_supabase.from('service_logs')
        .select('id,service_date,service_type,technician_name,job_description,branch_id,asset_id,branches(name),assets(name)')
        .eq('customer_id',activeCustomer.id)
        .order('service_date',{ascending:false});
