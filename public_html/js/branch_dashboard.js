/* ============================================================
   ...
   FIX 4: home tab — loadBdcRecentServices სწორი query
   FIX 5: loadBdcServices — assets ცალკე query
   FIX 6: bdcLoadA — ერთიანი კოდი asset_view.js-თან
         (ფილ. ბარათი + ფილ. dashboard — ერთი view)
   ============================================================ */

const OPENWEATHER_KEY = 'a155a131748709196243b52b5bf35b53';
let bdcOpen = true;

/* ── _doViewBranchDashboard ── */
...
function fillBdcHeader(b) {
    ...
    set('bdc-addr',  b.address||'—');
    set('bdc-phone', b.contact_phone||b.emergency_phone||'—');
    set('bdc-sla',   b.sla_response_hours?b.sla_response_hours+'h':'—');
    set('bdc-freq',  b.service_frequency||'—');
...
async function loadBdcKPIs() {
    ...
    if(assets){
        const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
        set('bdc-kpi-total', assets.length);
        set('bdc-kpi-ok',    assets.filter(a=>a.status==='Operational').length);
        set('bdc-kpi-maint', assets.filter(a=>a.status==='Maintenance_Required').length);
        set('bdc-kpi-down',  assets.filter(a=>a.status==='Down').length);
        set('bdc-kpi-ac',    assets.filter(a=>a.asset_class==='AC').length);
        set('bdc-kpi-ref',   assets.filter(a=>a.asset_class==='Refrigeration').length);
    }
    const {count}=await _supabase.from('service_logs').select('*',{count:'exact',head:true}).eq('branch_id',activeBranch.id);
    const kSvc=document.getElementById('bdc-kpi-svc');if(kSvc)kSvc.textContent=count??'0';
    ...
    const kLast=document.getElementById('bdc-kpi-last');if(kLast)kLast.textContent=last?.[0]?new Date(last[0].service_date).toLocaleDateString('en-GB',{day:'2-digit',month:'short'}):'—';
}

/* ── loadBdcWeather ── */
async function loadBdcWeather() {
    const lat=activeBranch.lat,lng=activeBranch.lng;
    const show=(id,show)=>{const e=document.getElementById(id);if(e){if(show)e.classList.remove('hidden');else e.classList.add('hidden');}};
    if(!lat||!lng){show('bdc-w-loading',false);show('bdc-w-error',true);show('bdc-w-widget-loading',false);show('bdc-w-widget-err',true);return;}
    try{
        ...
        const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
        show('bdc-w-loading',false);show('bdc-w-data',true);
        set('bdc-w-temp',temp+'°C');set('bdc-w-hum',hum+'%');set('bdc-w-wind',wind+' m/s');
        set('bdc-w-air',aqi);const aw=document.getElementById('bdc-w-air');if(aw)aw.style.color=aqiC;
        set('bdc-w-feels',feels+'°C');set('bdc-w-vis',vis);
        show('bdc-w-widget-loading',false);show('bdc-w-widget',true);
        set('bdc-w-widget-temp',temp);set('bdc-w-widget-desc',desc);
        const wi=document.getElementById('bdc-w-widget-icon');if(wi)wi.src=icon;
        set('bdc-w2-hum',hum+'%');set('bdc-w2-wind',wind+' m/s');
        ...
        const show2=(id,s)=>{const el=document.getElementById(id);if(el){if(s)el.classList.remove('hidden');else el.classList.add('hidden');}};
        show2('bdc-w-loading',false);show2('bdc-w-error',true);show2('bdc-w-widget-loading',false);show2('bdc-w-widget-err',true);
    }
}
...
/* ══════════════════════════════════════════════════════════════
   FIX 4: HOME TAB — assets ცალკე query
   ══════════════════════════════════════════════════════════════ */
async function loadBdcRecentServices() {
    ...
    /* FIX 4: branches(name) — assets ცალკე */
    const {data,error}=await _supabase.from('service_logs')
        .select('id,service_date,service_type,technician_name,asset_id,branches(name)')
        .eq('branch_id',activeBranch.id)
        .order('service_date',{ascending:false})
        .limit(5);

    if(error){el.innerHTML='<div class="text-xs text-red-400 italic text-center py-3">შეცდ.</div>';return;}
...
async function loadBdcAssetsMini() {
    const {data}=await _supabase.from('assets')
        .select('id,name,brand,model,status,asset_class,asset_type,cooling_capacity_kw,tag_number,last_service_date')
    ...
    const acB=document.getElementById('bdc-ac-badge'), refB=document.getElementById('bdc-ref-badge');
    if(acB)  acB.textContent=ac.length;
    if(refB) refB.textContent=ref.length;
...
/* ══════════════════════════════════════════════════════════════
   FIX 6: ASSETS TAB — ერთიანი view (სურათი 1 + სურათი 2)
   KPI strip + sidebar-style ფილტრი + ბარათები
   ══════════════════════════════════════════════════════════════ */
    ...
    <div class="space-y-4">
      ...
      <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
        <div class="flex items-center justify-between flex-wrap gap-3">
          <!-- class filter (სურ. 1-ის ხაზი) -->
          <div class="flex gap-2 flex-wrap">
            <button onclick="bdcFilterA('all',this)"
              class="bdc-af px-4 py-1.5 rounded-xl text-xs font-bold bg-slate-900 text-white border border-slate-900">ყველა</button>
            <button onclick="bdcFilterA('AC',this)"
              class="bdc-af px-4 py-1.5 rounded-xl text-xs font-bold text-slate-400 border border-slate-200 hover:border-blue-400 transition">კონდ. AC</button>
            <button onclick="bdcFilterA('Refrigeration',this)"
              class="bdc-af px-4 py-1.5 rounded-xl text-xs font-bold text-slate-400 border border-slate-200 hover:border-teal-400 transition">მაცივ.</button>
            <button onclick="bdcFilterA('Other',this)"
              class="bdc-af px-4 py-1.5 rounded-xl text-xs font-bold text-slate-400 border border-slate-200 hover:border-amber-400 transition">სხვა</button>
          </div>
          <div class="flex gap-2 flex-wrap">
            <select id="bdc-at-status" onchange="bdcLoadA()"
            ...
            <select id="bdc-at-type" onchange="bdcLoadA()"
              class="text-xs border border-slate-200 rounded-xl px-3 py-1.5 bg-slate-50 text-slate-600">
              <option value="">ყველა ტიპი</option>
              <option value="Split">Split</option><option value="Multi-Split">Multi-Split</option>
              <option value="Cassette">Cassette</option><option value="VRF">VRF/VRV</option>
              <option value="Chiller">Chiller</option><option value="Rooftop">Rooftop</option>
              <option value="AHU">AHU</option><option value="Reach-in">Reach-in</option>
              <option value="Walk-in">Walk-in</option><option value="Display Case">Display Case</option>
              <option value="Condensing Unit">Cond. Unit</option><option value="Blast Chiller">Blast Chiller</option>
              <option value="Boiler">Boiler</option><option value="DHW">DHW</option>
              <option value="Fan">Fan</option><option value="Pump">Pump</option><option value="Other">Other</option>
            </select>
          ...
          <button onclick="openAssetModal()"
            class="text-xs font-bold text-blue-600 border border-blue-200 px-4 py-1.5 rounded-xl hover:bg-blue-50 transition">+ ახალი</button>
        </div>
      </div>

      <!-- KPI strip (სურ. 1-ის მსგავსი) -->
...
function bdcFilterA(cls,btn) {
    window._bdcAC=cls;
    document.querySelectorAll('.bdc-af').forEach(b=>{
        b.className='bdc-af px-4 py-1.5 rounded-xl text-xs font-bold text-slate-400 border border-slate-200 hover:border-slate-400 transition';
    });
...
async function bdcLoadA() {
    ...
    const filtered=search?(all||[]).filter(a=>
        ...
        (a.tag_number||'').toLowerCase().includes(search)||
    ...
    if(kpiEl) kpiEl.innerHTML=[
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-4 text-center shadow-sm">
            ...
            <div class="text-[10px] text-slate-400 mt-1">${k.l}</div>
        </div>`).join('');
    ...
    /* FIX 6: ერთი საერთო card template — ორივე გვერდი */
    ...
    <div class="bg-white rounded-2xl border ${clB[a.asset_class]||'border-slate-200'} shadow-sm overflow-hidden hover:shadow-md transition-all group">
      <div class="h-36 bg-slate-50 relative overflow-hidden border-b ${clB[a.asset_class]||'border-slate-100'}">
        ${a.image_url
          ?`<img src="${a.image_url}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">`
          :`<div class="w-full h-full flex items-center justify-center text-slate-200">
              <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1">
                <rect x="2" y="7" width="20" height="10" rx="2"/><path d="M6 11h12"/>
              </svg>
            </div>`}
        <div class="absolute top-2 left-2">
          <span class="text-[9px] font-bold px-2 py-0.5 rounded-full ${sC[a.status]||'bg-slate-100 text-slate-500'}">${sL[a.status]||a.status}</span>
        </div>
        <div class="absolute top-2 right-2">
          <span class="text-[8px] font-bold px-2 py-0.5 rounded-full bg-white/80 text-slate-600 border border-slate-200">${a.asset_type||'—'}</span>
        </div>
      ...
      <div class="p-4">
        <div class="flex justify-between items-start mb-2">
          <div>
            <h4 class="text-sm font-black text-slate-900 leading-tight">${a.name}</h4>
            <p class="text-[10px] text-slate-400 mt-0.5">${[a.brand,a.model].filter(Boolean).join(' · ')||'—'}</p>
          </div>
        ...
        ${a.next_planned_service?`
        <div class="mb-3 px-2 py-1.5 rounded-lg ${new Date(a.next_planned_service)<new Date(Date.now()+30*86400000)?'bg-red-50 text-red-600':'bg-amber-50 text-amber-600'} text-[9px] font-bold">
          შემ. სერვ: ${new Date(a.next_planned_service).toLocaleDateString('ka-GE',{day:'2-digit',month:'short',year:'numeric'})}
        </div>`:''}
        <div class="flex gap-2">
          <button onclick='openBdcAsset(${JSON.stringify(a)})'
            class="flex-1 bg-slate-900 text-white py-2 rounded-xl text-[9px] font-black uppercase hover:bg-blue-600 transition">
            სერვ. ისტ.
          </button>
          <button onclick='openAssetModal(${JSON.stringify(a)})'
            class="flex-1 border border-slate-200 text-slate-500 py-2 rounded-xl text-[9px] font-black uppercase hover:bg-slate-50 transition">
            რედ.
          </button>
...
/* ══════════════════════════════════════════════════════════════
   FIX 5: SERVICES TAB — assets ცალკე query
   ══════════════════════════════════════════════════════════════ */
async function loadBdcServices() {
    ...
    let q=_supabase.from('service_logs')
        .select('id,service_date,service_type,technician_name,job_description,asset_id,branches(name)')
        .eq('branch_id',activeBranch.id)
        .order('service_date',{ascending:false});
    ...
    <table class="w-full text-xs border-collapse">
      ...
      <tr class="border-b border-slate-50 hover:bg-slate-50">
        <td class="py-2 px-2 text-slate-500 whitespace-nowrap">${new Date(s.service_date).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}</td>
        ...
        <td class="py-2 px-2 text-slate-500">${aMap[s.asset_id]||'—'}</td>
        <td class="py-2 px-2 text-slate-400 truncate max-w-[200px]">${s.job_description?s.job_description.substring(0,60)+'...':'—'}</td>
      </tr>`).join('')}
      </tbody>
    </table>`;
}
...
function loadBdcAssetsFull()    { loadBdcAssetsPanel(); }
